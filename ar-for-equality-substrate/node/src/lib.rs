// filepath: ar-for-equality-substrate/ar-for-equality-substrate/node/src/lib.rs
...
pub mod pallet_campaign_management {
    ...
    pub enum Event<T: Config> {
        CampaignCreated { campaign_id: T::Hash },
        CampaignFunded { campaign_id: T::Hash, amount: u128 },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_campaign(origin: OriginFor<T>, title: Vec<u8>, target: u128) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let campaign_id = T::Hash::from([0u8; 32]); // Placeholder for campaign ID generation
        ...
        #[pallet::weight(10_000)]
        pub fn fund_campaign(origin: OriginFor<T>, campaign_id: T::Hash, amount: u128) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let mut campaign = Campaigns::<T>::get(campaign_id).ok_or(Error::<T>::CampaignNotFound)?;

            ensure!(T::Currency::free_balance(&who) >= amount.into(), Error::<T>::InsufficientFunds);
